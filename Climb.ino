...
float controller_flag = 1;  // Define the controller flag
...
const int numReadings = 10; // Number of readings to consider for detecting climbing
float pitchReadings[numReadings]; // Array to hold pitch readings
int readIndex = 0; // Current reading index
float totalPitchChange = 0; // Total change in pitch
bool isClimbing = false; // Climbing status
...
void setup() {
  ...
  // Initialize pitch readings array
  ...
  if (dataFile) {
    ...
    dataFile.println("================================================================");
    dataFile.println((String) "Time [s] " + "Pos [deg] " + "Actual Pos [deg] " + "Velocity [deg/s] " + "ax " + "ay " + "az " + "gx " + "gy " + "gz " + "amplitude_left " + "amplitude_right " + "frequency_left " + "frequency_right " + "isClimbing");
    dataFile.close();
  } else {
...
void loop() {
  ...
  datum.isClimbing = isClimbing;

  storeData(datum);

  // isClimbing = detectClimbing(gy);

  // Print the climbing status
  // if (isClimbing) {
  //   Serial.println("The biped is climbing.");
  //   controller_flag = 2;
  // } else {
  //   Serial.println("The biped is not climbing.");
  //   controller_flag = 1;
  // }
...
bool detectClimbing(float currentPitch) {
  // Store the current reading
  pitchReadings[readIndex] = currentPitch;
  readIndex = (readIndex + 1) % numReadings;

  // Calculate total pitch change
  totalPitchChange = 0;
  for (int i = 0; i < numReadings - 1; i++) {
    totalPitchChange += pitchReadings[i] - pitchReadings[i + 1];
  }

  // Check if the total pitch change indicates climbing
  float pitchChangeThreshold = 2.0; // Adjust threshold value as necessary
  if (totalPitchChange > pitchChangeThreshold) {
    return true; // The biped is climbing
  } else {
    return false; // The biped is not climbing
  }
}
...
float climbingCoeff = 0.5;
float climbingOffset = (65 - gy) * climbingCoeff;

void climb_controller() {
  if (gx > 180 && gx < 360 - reasonable_alpha_z_range) {
    A_left = bigAmp;
    A_right = smallAmp;
  } else if (gx > reasonable_alpha_z_range && gx < 180) {
  ...
  A_left = A_left - climbingOffset;
  A_right = A_right - climbingOffset;
  f_left = 1.70;
  f_right = 1.70;
}
...
void writeBufferToSD() {
  ...
  if (dataFile) {
    for (int i = 0; i < bufferIndex; i++) {
      ...
      dataFile.print(",");
      dataFile.print(dataBuffer[i].f_right);
      dataFile.print(",");
      dataFile.println(dataBuffer[i].isClimbing);
    }
